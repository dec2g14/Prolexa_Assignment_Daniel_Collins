%%% Content:
% Predicate word relation
% Sentence clause relation
% Phrase structure (verb phrases)
% Properties
% Determiners
% Proper nouns (names, things)
% Question relations
...
% User defined functions for mapping and negation (Simply Logical Ch 8.1)
:- op(600, xfy, '=>').
:-op(900, fy, not). % Test 3: Negation Operator
...
pred(bird,    1,[n/bird]). %TEST 4
pred(penguin, 1,[n/penguin]). %TEST 4
...
% Test 3 Negation
sentence1([(not(L):-true)]) --> proper_noun(N,X),verb_phrase(N,not(X=>L)). %Test 3
sentence1([(L:-true)]) --> proper_noun(N,X),verb_phrase(N,X=>L).


verb_phrase(s,M) --> [is],property(s,M).
% Test 3 - Negation: New Verb Phrase Format
verb_phrase(s,not(M)) --> [is],[not],property(s,M). % Test 3
verb_phrase(s,not(M)) --> [not],property(s,M). %Test 3
...
question1(not(Q)) --> [who],verb_phrase(s,not(_X=>Q)). % Test 3
question1(Q) --> [is], proper_noun(N,X),property(N,X=>Q).
question1(not(Q)) --> [is],proper_noun(N,X),verb_phrase(N,not(X=>Q)). % Test 3
question1(Q) --> [does],proper_noun(_,X),verb_phrase(_,X=>Q).
question1(not(Q)) --> [does],proper_noun(_,X),verb_phrase(_,not(X=>Q)).% Test 3
